...
//Prevents blurry output
renderer.setSize( window.innerWidth,window.innerHeight ); 
//Sets renderer size to the size of the window
renderer.setClearColor(0xA9F5F2, 1);
 //Makes the background color of the scene blue
renderer.shadowMapEnabled = true;
renderer.shadowMapSoft = true;
document.body.appendChild( renderer.domElement ); 
//Attaches renderer to DOM (initializes renderer)

var scene = new THREE.Scene(); 
//Creates an empty scene where we are going to add our objects

var camera = new THREE.PerspectiveCamera( FOV,ASPECT,NEAR,FAR ); 
//Creates a camera
camera.up.set( 0,0,1 ); 
//Sets the camera the correct direction
...
var width  = 40; //width of ground
var length = 40; //length of ground

var geometry = new THREE.PlaneGeometry( width, length); //ThreeJS function to create plane geometry
var texture;
texture = new THREE.TextureLoader().load( "https://www.dropbox.com/s/rro7rfw1i34lmxp/Proyek%20Baru%20161%20%5B19202F3%5D.png?dl=0" );
...
myArray=[];
myArray.push(spawnBody(0xff00c7));
myArray.push(spawnBody(0xff00c7));
...
swal( "Game Over 🌵","Kamu kecemplung sungai!\nSkor tertinggi: " + eatVar + "\n\n😄 Trimakasih sudah main! 😄");
scoreId.innerHTML = "Main lagi?? Tekan tombol nya";
...
for(qTimer=0;qTimer<zTimer.length;qTimer++){
	...
	swal( "Game Over 🌵","Kamu menggigit Ekor kmu sendiri!\nSkor tertinggi: " + eatVar + "\n\n😄 Trimakasih sudah main! 😄");
...
swal("😅Welkome😅", "Selamat datang user ballbot, silahkan bermain game ini\nInstruksi bermain: Ketik tombol kanan kiri untuk start, dan arah kan ularnya ke jalan yang benar, jangan sesatkan\nMaafin gw ya, ular nya buntung, kepala nya plintis banget, gada rambut nya" );
pauseVar=1;
scoreId.innerHTML = "Tekan tombol untuk mulai";
